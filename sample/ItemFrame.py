...
class ItemFrame(tk.Frame):
    def __init__(self, frame, currentLocation, locations, itemNames=[]):
        super().__init__()
        ...
        locationOptionMenu = tk.OptionMenu(
            self,
            tk.StringVar(value=currentLocation),
            *locations,
            command=lambda event: self.event_generate("<<setLocation>>")
        )
        locationOptionMenu.grid(column=1,row=1)
        # Item Name
        self.itemName = tk.Entry(self)
        self.itemName.grid(column=2,row=0)
        # Item Aisle
        self.itemAisle = tk.Entry(self)
        self.itemAisle.grid(column=2,row=1)
        # Add Item Button
